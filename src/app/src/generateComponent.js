...
export default function generateComponent(basePath, compPath, { type, name = 'template' }) {
  ...
  if (fs.pathExistsSync(absolutPath)) {
    ...
    const template = require(`../component/${type}/${name}.js`).default
    fs.ensureDirSync(dirPath)
    if (type !== 'components') key += upperFirst(type)
    fs.writeFileSync(absolutPath, template(key))
    if (type === 'components') {
      const cssTemplate = require(`../common/css/template.js`).default
      fs.writeFileSync(path.join(dirPath, 'index.scss'), cssTemplate(key))
    }
