          ...
          if (typeof loader === 'function') {
            ...
            if (file.name.split('.') && file.name.split('.')[1] === 'js') {
              ...
              if (file.importPath) {
                ...
                var resolvePathCode = compileCode.replace(/'components\//g, '\'' + path.join(config.projectPath, 'components') + '/');
            ...
            if (component && component.default) {
              ...
              // 补全的快捷键核心定义
              // ------------------------------------
              var componentName = component.default && component.default.displayName || file.importName;
              var matchDisplayname = componentName.match(/\(((\w)*?)\)/g);
              var displayName = matchDisplayname.length && matchDisplayname[0].replace(/\(|\)/g, '');
